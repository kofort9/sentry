...
class WorkflowObserver(ABC):
    ...
    def log_workflow_start(
        self, workflow_name: str, input_data: Any, metadata: Dict[str, Any] = None
    ):
        """Log workflow start."""
        pass
...
class ConsoleObserver(WorkflowObserver):
    ...
    def log_event(self, event: ObservabilityEvent):
        ...
        message = (
            f"[{event.event_type.value}] {event.source}: {json.dumps(event.data, default=str)}"
        )
    ...
    def log_workflow_start(
        ...
        event = ObservabilityEvent(
            ...
            data={
                "input_summary": (
                    str(input_data)[:100] + "..." if len(str(input_data)) > 100 else str(input_data)
                )
            },
    ...
    def log_workflow_end(
        ...
        event = ObservabilityEvent(
            ...
            data={
                ...
                "result_summary": (
                    str(result)[:100] + "..." if result and len(str(result)) > 100 else str(result)
                ),
                "error": error,
            },
    ...
    def log_llm_interaction(
        ...
        event = ObservabilityEvent(
            timestamp=datetime.datetime.now().isoformat(),
            event_type=(
                EventType.LLM_REQUEST if llm_role in ["system", "user"] else EventType.LLM_RESPONSE
            ),
            source=agent_name,
            data={
...
class MemoryObserver(WorkflowObserver):
    ...
    def log_workflow_end(
        ...
        event = ObservabilityEvent(
            ...
            data={
                ...
                "duration_seconds": duration,
                "result_summary": (
                    str(result)[:200] + "..." if result and len(str(result)) > 200 else str(result)
                ),
                "error": error,
            },
            level=LogLevel.INFO if success else LogLevel.ERROR,
    ...
    def get_recent_events(
        self, count: int = 50, event_type: EventType = None
...
class AgentLogger:
    ...
    def log_llm_call(
        self, role: str, content: str, model: str = None, metadata: Dict[str, Any] = None
    ):
        """Log LLM interaction."""
