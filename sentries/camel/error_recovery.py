...
class ErrorRecoverySystem:
    ...
    def classify_error(self, error: Exception, context: Dict[str, Any] = None) -> ErrorInfo:
        ...
        # Model/LLM-related errors
                ...
                ['model', 'llm', 'api', 'openai', 'anthropic', 'groq', 'ollama']):
        ...
        # Parsing errors
                ...
                ['json', 'parse', 'decode', 'syntax']):
            category = ErrorCategory.PARSING
            severity = ErrorSeverity.MEDIUM
            
        # Configuration errors
        elif any(keyword in error_message.lower() for keyword in 
                ['config', 'environment', 'missing', 'not found']):
    ...
    def attempt_recovery(self, error_info: ErrorInfo) -> bool:
        ...
        if error_info.retry_count >= self.max_retries:
            logger.warning(f"Max retries ({self.max_retries}) reached for error: {error_info.message}")
            return False
        
        error_info.recovery_attempted = True
        error_info.retry_count += 1
        
        logger.info(f"Attempting recovery for {error_info.category.value} error (attempt {error_info.retry_count})")
        
        # Get recovery strategy for this error category
        recovery_func = self.recovery_strategies.get(error_info.category, self._recover_generic_error)
    ...
    def with_recovery(self, operation: Callable, context: Dict[str, Any] = None, 
                     custom_max_retries: Optional[int] = None) -> Any:
        """
...
def with_error_recovery(context: Dict[str, Any] = None):
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            return global_error_recovery.with_recovery(
