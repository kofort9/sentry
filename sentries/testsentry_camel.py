    ...
    return {
        ...
        "success_metrics": {
            "planner_interactions": next(
                (agent["interactions"] for agent in workflow_history.get("agents", []) 
                 if agent["name"] == "planner"), 0
            ),
            "patcher_interactions": next(
                (agent["interactions"] for agent in workflow_history.get("agents", []) 
                 if agent["name"] == "patcher"), 0
            )
        }
    }
...
**Generated by:** TestSentry CAMEL Edition with resource-conscious multi-agent coordination

---
*This PR was automatically created by TestSentry CAMEL after intelligent multi-agent analysis and fix generation.*"""
...
def main() -> None:
    ...
    coordinator = CAMELCoordinator(
        planner_model=str(MODEL_PLAN),
        patcher_model=str(MODEL_PATCH)
    )
