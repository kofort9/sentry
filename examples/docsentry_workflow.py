...
from sentries.framework import (
    BaseAgent, AgentConfig, AgentRole, LLMAgent, ToolAgent,
    BaseTool, ToolCategory, ToolMetadata, ToolRegistry,
    WorkflowBuilder, WorkflowEngine, WorkflowObserver, MemoryObserver,
    ErrorRecoverySystem, MockLLMWrapper, LLMConfig, LLMProvider
)
...
class DocumentGenerationTool(BaseTool):
    ...
    def execute(self, plan: Dict[str, Any], current_content: str, style_guide: str = None) -> Dict[str, Any]:
    ...
    def process(self, input_data: Any, context: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            messages = [
                ...
                {"role": "user", "content": f"Analyze this documentation file: {input_data}\n\nTool analysis: {tool_result}"}
            ]
  ...
  "estimated_effort": "low/medium/high"
    ...
    def process(self, input_data: Any, context: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            messages = [
                {"role": "system", "content": self.config.system_message},
                {"role": "user", "content": f"Create an update plan based on this analysis: {analysis_result}"}
            ]
...
class DocumentUpdaterAgent(ToolAgent):
    ...
    def process(self, input_data: Any, context: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        try:
            plan = input_data
            current_content = context.get("current_content", "# Example Documentation\n\nThis is version 1.2.0")
            
            # Generate updated content
            generation_tool = self.get_tool("DocumentGenerationTool")
            generation_result = generation_tool.execute(
                plan=plan,
...
def create_docsentry_workflow() -> WorkflowEngine:
    ...
    mock_llm = MockLLMWrapper(responses=[
        '{"analysis": "Documentation contains outdated version references", "priority_issues": ["version_mismatch"], "recommendations": ["update_versions"], "confidence": 0.9}',
        '{"plan": "Update version references and fix broken links", "target_sections": ["Installation"], "update_strategy": "systematic_update", "estimated_effort": "low"}'
    ])
    ...
    # Create agents
    analyzer = DocumentAnalyzerAgent("mock-model", llm_wrapper=mock_llm, tool_registry=tools, observer=observer)
    planner = DocumentPlannerAgent("mock-model", llm_wrapper=mock_llm, tool_registry=tools, observer=observer)
    updater = DocumentUpdaterAgent(tool_registry=tools, observer=observer)
    
    # Build workflow
    workflow = (WorkflowBuilder("DocSentry", "Automated documentation updating workflow")
...
def run_docsentry_example():
    ...
    if hasattr(workflow.observer, 'get_llm_interactions'):
        ...
        for interaction in interactions[:2]:  # Show first 2
            print(f"  - {interaction['agent_name']} ({interaction['llm_role']}): {interaction['content'][:50]}...")
