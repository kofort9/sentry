...
try:
    ...
    from sentries.camel.error_recovery import global_error_recovery
    from sentries.runner_common import MODEL_PATCH, MODEL_PLAN
...
def demo_error_recovery():
    ...
    for error, context in test_errors:
        ...
        status_icon = "✅" if recovery_success else "❌"
        status_text = "Successful" if recovery_success else "Failed"
        print(f"   {status_icon} Recovery: {status_text}")
...
"""
    ...
    if "error_recovery_summary" in result:
        ...
        recovery_rate = recovery.get("recovery_rate", 0)
        print(f"   Error Recovery: {total_errors} errors, {recovery_rate:.1%} recovery rate")
