...
class TestPlannerAgentLearning:
    ...
    def test_historical_context_integration(self):
        ...
        self.planner.conversation_history = [
            {
                "input": {"failure_type": "ASSERT_MISMATCH", "test_file": "tests/test_math.py"},
                "learning_context": {
    ...
    def test_risk_assessment(self):
        ...
        test_cases = [
            ...
            {"error_message": "Simple assertion failed", "expected_risks": []},
        ]
    ...
    def test_conversation_history_enhancement(self, mock_datetime):
        ...
        self.planner.llm.generate.return_value = json.dumps(
            {"plan": "Fix syntax error", "target_files": ["tests/test_example.py"]}
        )

        # Act
        self.planner.analyze_and_plan(test_output)
    ...
    def test_plan_enhancement_with_insights(self):
        ...
        original_plan = {"plan": "Fix assertion", "target_files": ["tests/test.py"]}
...
class TestPlannerAgentIntegration:
    ...
    def test_learning_across_multiple_sessions(self):
        ...
        sessions = [
            {"failure_type": "ASSERT_MISMATCH", "success": True, "test_file": "tests/test_math.py"},
            {"failure_type": "ASSERT_MISMATCH", "success": True, "test_file": "tests/test_calc.py"},
            {
        ...
        for i, session in enumerate(sessions):
                    ...
                    planner.conversation_history.append(
                        {
                            ...
                            "learning_context": {
                                ...
                                "successful_strategies": (
                                    ["successful_plan"] if prev_session["success"] else []
                                ),
                            },
                        }
                    )
            ...
            planner.llm.generate.return_value = json.dumps(
                {"plan": f"Fix session {i}", "target_files": [session["test_file"]]}
            )
        ...
        # Total sessions include the seed history plus the analyze_and_plan calls
        assert final_patterns["total_planning_sessions"] >= 3  # At least 3 sessions
        common_types = final_patterns["common_failure_types"]
        assert "ASSERT_MISMATCH" in common_types
        assert "IMPORT_ERROR" in common_types
...
"""
        ...
        # The learning_context accounts for the prior history plus the new session
