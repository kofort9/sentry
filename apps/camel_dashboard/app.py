...
try:
    ...
    from sentries.runner_common import MODEL_PLAN, MODEL_PATCH, get_logger
...
class CAMELDashboard:
        ...
        try:
            if not self.coordinator:
                ...
                self.coordinator = CAMELCoordinator(
                    ...
                    llm_logger=self.log_llm_interaction
                )
                logger.info("🐫 CAMEL coordinator initialized successfully and persisted in session state")
            return True
    ...
    def log_llm_interaction(self, agent_type: str, role: str, content: str, 
                           model: str = None, metadata: Dict[str, Any] = None):
...
"""
                ...
                self.log_llm_interaction(
                    ...
                    metadata={
                        "result_summary": {
                            ...
                            "error_count": len(result.get("error_recovery_summary", {}).get("error_history", [])),
                        }
                    }
                )
            ...
            with st.expander("✅ Validation Details"):
                for i, attempt in enumerate(result["validation_attempts"], 1):
                    ...
                    if validation.get("valid"):
                        ...
                        st.error(f"Attempt {i}: ❌ Issues: {', '.join(validation.get('issues', []))}")
    ...
    def render_architecture_overview(self):
        ...
        st.markdown(
            """
            - **Trigger**: Failing pytest output enters the coordinator as the workflow seed.
            - **Agents**: Planner creates structured plans/context packs; Patcher turns them into diffs.
            - **Patch Engine**: Applies JSON/diff operations and hands results to repo + validators.
            - **Validation Loop**: Validation failures go through global error recovery before retrying.
            - **Observability**: Coordinator streams logs/metrics into this dashboard for human review.
            """
        )
    ...
    def render_error_log(self):
        ...
        if recent_errors:
            ...
            for error in recent_errors[-5:]:  # Show last 5 errors
                ...
                recovery_status = "✅ Recovered" if error.get('recovery_successful') else "❌ Not Recovered"
                retry_info = f"(Retries: {error.get('retry_count', 0)})" if error.get('retry_count', 0) > 0 else ""
                
                with st.expander(f"{icon} {error.get('category', 'Unknown')} - {time_ago.total_seconds()//60:.0f}m ago {recovery_status} {retry_info}"):
                    col1, col2 = st.columns(2)
                    ...
                    with col2:
                        st.write(f"**Recovery Attempted:** {'Yes' if error.get('recovery_attempted') else 'No'}")
                        st.write(f"**Recovery Successful:** {'Yes' if error.get('recovery_successful') else 'No'}")
    ...
    def render_llm_sessions(self):
        ...
        if logs:
            ...
            for i, log in enumerate(reversed(logs[-20:])):  # Show last 20 interactions
                ...
                header = f"{agent_icon} **{agent_type.title()}** {role_icon} *{role}* - {time_ago.total_seconds()//60:.0f}m ago"
            ...
            st.info("ℹ️ No LLM session logs available. Logs will appear here when agents interact with LLMs.")
            ...
            if st.button("💾 Export History"):
                if st.session_state.workflow_history:
                    ...
                    st.download_button(
                        "⬇️ Download CSV",
                        csv,
                        file_name=f"camel_workflow_history_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
                        mime="text/csv"
                    )
                else:
